...
export default function CreatePayment({ payment, enrolments }: Props) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        },
        {
            ...
            href: isEdit ? `/management-pemasukan/edit/${payment?.id}` : '/management-pemasukan/create',
        },
    ];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (isEdit) {
            put(`/management-pemasukan/update/${payment?.id}`);
        } else {
            post('/management-pemasukan');
        }
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={isEdit ? "Edit Payment" : "Tambah Payment"} />
            <div className="p-4 sm:p-6">
                {/* Header */}
                <div className="flex items-start sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
                    <Link href="/management-payment" className="shrink-0">
                    ...
                    <div className="min-w-0">
                        <h1 className="text-xl sm:text-2xl font-bold text-foreground truncate">
                            {isEdit ? 'Edit Payment' : 'Tambah Payment Baru'}
                        </h1>
                        <p className="text-sm sm:text-base text-muted-foreground">
                            {isEdit ? 'Perbarui data payment' : 'Catat pembayaran dari member'}
                        </p>
                    </div>
                </div>
